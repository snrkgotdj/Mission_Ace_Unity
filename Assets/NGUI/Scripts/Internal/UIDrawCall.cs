...
public class UIDrawCall : MonoBehaviour
{
	...
	//---pat-------
	[System.NonSerialized] bool mAcePolygonClip = false;
	...
	void CreateMaterial ()
	{
		...
		if (panel != null && panel.clipping == Clipping.TextureMask)
		{
            ...
            //if(shader != null) Debug.LogWarning("clipping = Clipping.TextureMask / shader : " + shader.name);
		}
		//----pat-------------
		else if ( panel != null && panel.clipping == Clipping.AcePolygon )
		{
			mAcePolygonClip = true;
			shader = Shader.Find ("AceHidden/" + shaderName);
		}
		else if (mClipCount != 0)
		{
			shader = Shader.Find("Hidden/" + shaderName + " " + mClipCount);
			if (shader == null) shader = Shader.Find(shaderName + " " + mClipCount);
            //if (shader != null) Debug.LogWarning("mClipCount != 0 / shader : " + shader.name);
			// Legacy functionality
			if (shader == null && mClipCount == 1)
			{
				mLegacyShader = true;
				shader = Shader.Find(shaderName + soft);
                //if (shader != null) Debug.LogWarning("mClipCount == 1 / shader : " + shader.name);
            }else
            {
                //if (shader != null) Debug.LogWarning("mClipCount "+ mClipCount.ToString()+ "/ shader : " + shader.name);
            }
		...
		if (shader == null)
        {
            ...
            //if (shader != null) Debug.LogWarning("shader == null / shader : " + shader.name);
        }
            

        //if (shader != null) Debug.LogWarning("Shader : " + shader.name);
	...
	void UpdateMaterials ()
	{
		...
		if (mRebuildMat || mDynamicMat == null || mClipCount != panel.clipCount || mTextureClip != (panel.clipping == Clipping.TextureMask)
			//----pat------
			|| mAcePolygonClip != (panel.clipping == Clipping.AcePolygon ))
		{
			RebuildMaterial();
			mRebuildMat = false;
		}
	...
	void OnWillRenderObject ()
	{
		...
		if ( (mDynamicMat == null || mClipCount == 0 ) && panel.clipping != Clipping.AcePolygon) return;
