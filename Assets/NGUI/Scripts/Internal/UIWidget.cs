...
public class UIWidget : UIRect
{
    ...
    public void CheckLayer()
    {
        ...
        if (panel != null && panel.gameObject.layer != gameObject.layer)
        {
            Debug.LogWarning("You can't place widgets on a layer different than the UIPanel that manages them.\n" +
                             "If you want to move widgets to a different layer, parent them to a new panel instead." + gameObject.name, this);
	...
	{
#if UNITY_EDITOR
        if (!Application.isPlaying && GetComponent<UIPanel>() != null)
		{
			//Debug.LogWarning("Widgets and panels should not be on the same object! Widget must be a child of the panel.", 21, Color.red, this);
		}
		else if (!Application.isPlaying && GetComponents<UIWidget>().Length > 1)
		{
			//Debug.LogWarning("You should not place more than one widget on the same object. Weird stuff will happen!", 21, Color.red, this);
		}
#endif
		CreatePanel();
	}
