...
static public class NGUIText
{
	...
	//static public float fontDefaultScale = 10;
	...
	static public bool ParseSymbol (string text, ref int index)
	{
		...
		return ParseSymbol(text, ref index, null, false, ref n, ref bold, ref italic, ref underline, ref strikethrough, ref ignoreColor, ref new_fontsize);
	}
		...
		ref int new_fontsize)
	{
		...
		if (text[index + 6] == ']')
		{
			string sub7 = text.Substring(index, 7).ToLower();
		...
		if (text [index + 5] == '=') 
		{
			string sub1_4 = text.Substring(index + 1, 4).ToLower();
			if ( sub1_4 == "size")
			{
				if ( text[index + 7] == ']' )
				{
					string subNum = text.Substring (index + 6, 1);
					int fs = 0;
					int.TryParse (subNum, out fs);
					if ( fs != 0 )
					{
						new_fontsize = fs;
						index += 8;
				...
				else if ( index + 9 <= length && text[index + 8] == ']' )
				{
					string subNum = text.Substring (index + 6, 2);	
					int fs = 0;
					int.TryParse (subNum, out fs);
					if ( fs != 0 )
					{
				...
				else if ( index + 10 <= length && text[index + 9] == ']' )
				{
					string subNum = text.Substring (index + 6, 3);	
					int fs = 0;
					int.TryParse (subNum, out fs);
					if ( fs != 0 )
					{
		...
		if (text != null)
		{
			for (int i = 0, imax = text.Length; i < imax; )
			{
				...
				if (c == '[')
				{
					...
					if (ParseSymbol(text, ref retVal, null, false, ref sub, ref bold, ref italic, ref underline, ref strikethrough, ref ignoreColor, ref new_fontsize))
					{
		...
		if (!string.IsNullOrEmpty(text))
		{
			...
			for (int i = 0; i < textLength; ++i)
			{
				...
				if (encoding && ParseSymbol(text, ref i, mColors, premultiply, ref subscriptMode, ref bold,
					ref italic, ref underline, ref strikethrough, ref ignoreColor, ref new_fontsize))
				{
					if (new_fontsize == 0) 
					{
				...
				{
					scale = fontScale / fontSize * new_fontsize;
				}
	...
	static public bool WrapText (string text, out string finalText, bool keepCharCount, bool wrapLineColors, bool useEllipsis = false)
	{
		...
		//-----pat---------------
		float sizeOfDot = GetGlyphWidth ('.', '.', fontScale);
		if ( sizeOfDot <= 0.0f )
			//&& text.Contains("헤이워"))
		{
			sizeOfDot = 5.0f;
		...
		float ellipsisWidth = useEllipsis ? finalSpacingX + sizeOfDot * 3f : finalSpacingX;
		int lastValidChar = 0;
	...
	static public void Print (string text, List<Vector3> verts, List<Vector2> uvs, List<Color> cols)
	{
		...
		//fontDefaultScale = fontScale /  finalSize;
		//float ori_scale = fontScale;
		...
		for (int i = 0; i < textLength; ++i)
		{
			...
			else // No symbol present
			{
				...
				if (new_fontsize != fontSize) 
				{
					...
					float pd = fontScale * pixelDensity;
					float dLine = baseline / (sizeF) * new_fontsize;
					glyph.v0.y += -baseline * pd + dLine * pd;
					glyph.v1.y += -baseline * pd + dLine * pd;
				}
